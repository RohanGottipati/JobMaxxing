...
export function DocumentationBrowser({ articles }: { articles: DocumentationArticle[] }) {
  ...
  return <div className="grid gap-7"><div className="surface-grid relative max-w-2xl rounded-xl border border-border bg-card p-3 shadow-paper"><Search aria-hidden className="absolute left-6 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search the product guide" className="h-10 pl-9" /></div>{categories.map((category) => <section key={category} className="grid gap-3"><h2 className="micro-label text-muted-foreground">{category}</h2><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{results.filter((article) => article.category === category).map((article) => <Link key={article.slug} href={`/documentation/${article.slug}`} className="group"><Card className="interactive-card h-full bg-card group-hover:border-border-strong group-hover:bg-elevated group-hover:shadow-[0_12px_28px_-14px_rgb(41_40_36/0.4)]"><CardContent className="flex h-full flex-col"><div className="flex items-start justify-between"><span className="grid size-9 place-items-center rounded-md border border-border bg-parchment text-primary"><BookOpenText aria-hidden className="size-4" /></span><Badge variant="outline"><Clock aria-hidden className="mr-1 size-3" />{article.readTime}</Badge></div><h3 className="mt-4 font-semibold tracking-[-0.02em]">{article.title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{article.description}</p><span className="mt-auto flex items-center gap-1.5 pt-5 text-sm font-medium text-primary">Read guide<ArrowRight aria-hidden className="size-4 transition-transform duration-200 group-hover:translate-x-1" /></span></CardContent></Card></Link>)}</div></section>)}{results.length === 0 ? <div className="surface-grid rounded-xl border border-dashed border-border-strong bg-parchment/35 p-10 text-center"><p className="font-medium">No guide matches that search.</p><p className="mt-1 text-sm text-muted-foreground">Try a feature name such as resumes, applications, or uploads.</p></div> : null}</div>;
}
