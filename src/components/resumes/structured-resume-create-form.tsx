...
export function StructuredResumeCreateForm({ error, returnTo = "resumes" }: { error?: string; returnTo?: "onboarding" | "resumes" }) {
  return <div className="grid gap-5">
    ...
    <form action={createStructuredMasterResumeAction} className="grid gap-5">
      ...
      <fieldset><legend className="mb-3 text-sm font-medium">Choose a starting template</legend><RadioGroup name="template_id" defaultValue={RESUME_TEMPLATES[0]?.id} className="grid gap-3 sm:grid-cols-2">{RESUME_TEMPLATES.map((template) => <label key={template.id} htmlFor={`template-${template.id}`} className="group grid cursor-pointer grid-cols-[auto_1fr] gap-3 rounded-xl border border-border bg-card p-3 transition-colors has-data-[state=checked]:border-primary has-data-[state=checked]:ring-2 has-data-[state=checked]:ring-primary/20"><RadioGroupItem id={`template-${template.id}`} value={template.id} className="mt-1" /><span><span className="block font-medium">{template.name}</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">{template.description}</span><span className="mt-3 block h-28 rounded border border-border bg-white p-3" aria-hidden><span className="mx-auto block h-2 w-2/3 rounded bg-neutral-800" /><span className="mx-auto mt-2 block h-1 w-1/2 rounded bg-neutral-300" /><span className="mt-4 block h-px bg-neutral-500" /><span className="mt-2 block h-1 w-full rounded bg-neutral-200" /><span className="mt-2 block h-1 w-4/5 rounded bg-neutral-200" /><span className="mt-4 block h-px bg-neutral-500" /></span></span></label>)}</RadioGroup></fieldset>
      <div className="flex flex-wrap gap-2"><SubmitButton pendingLabel="Creating…">Create structured resume</SubmitButton><Link href="/resumes/import" className={buttonVariants({ variant: "outline" })}>Import existing resume</Link><Button asChild variant="ghost"><Link href="/resumes/new?mode=latex">Create with LaTeX</Link></Button><Button asChild variant="ghost"><Link href="/resumes/new?mode=legacy">Use legacy text editor</Link></Button></div>
    </form>
  </div>;
}
