-- Manual Overleaf project synchronization.
--
-- The extension uploads files to unique staging paths first. This RPC then
-- swaps the source, supporting-file metadata, and per-Overleaf-project cursor
-- in one transaction. Storage objects that are no longer referenced enter a
-- durable cleanup queue that the application drains after the transaction.

-- Whole-project sync can include additional .tex files and nested folders.
alter table public.latex_document_assets
  add column content_hash text,
  add constraint latex_document_assets_content_hash_check
    check (content_hash is null or content_hash ~ '^[0-9a-f]{64}$');

alter table public.latex_document_assets
  drop constraint latex_document_assets_file_name_shape_check,
  drop constraint latex_document_assets_file_name_extension_check;

alter table public.latex_document_assets
  add constraint latex_document_assets_file_name_shape_check
    check (
      pg_catalog.length(file_name) between 1 and 240
      and file_name ~ '^[A-Za-z0-9][A-Za-z0-9._/-]{0,239}$'
      and file_name not like '%..%'
      and file_name not like '%//%'
      and file_name not like '%/./%'
      and pg_catalog.strpos(file_name, pg_catalog.chr(92)) = 0
      and pg_catalog.right(file_name, 1) <> '/'
    ),
  add constraint latex_document_assets_file_name_extension_check
    check (
      pg_catalog.lower(file_name) ~ '\.(tex|png|jpg|jpeg|pdf|ttf|otf|cls|sty|bib|bst)$'
      and pg_catalog.lower(file_name) not in ('main.tex', 'jobmaxxing-sync.json')
    );

create table private.latex_overleaf_sync_links (
  ...
  origin_row_version bigint not null,
  created_at timestamptz not null default now(),
  constraint latex_overleaf_sync_links_one_parent_check
    check (num_nonnulls(resume_id, resume_version_id, cover_letter_id) = 1),
  constraint latex_overleaf_sync_links_manifest_version_check
    check (manifest_version = 1),
  constraint latex_overleaf_sync_links_origin_version_check
    check (origin_row_version >= 0),
  constraint latex_overleaf_sync_links_resume_user_fkey
    foreign key (resume_id, user_id)
    references public.resumes (id, user_id) on delete cascade,
  constraint latex_overleaf_sync_links_version_user_fkey
    foreign key (resume_version_id, user_id)
    references public.resume_versions (id, user_id) on delete cascade,
  constraint latex_overleaf_sync_links_cover_letter_user_fkey
    foreign key (cover_letter_id, user_id)
    references public.cover_letters (id, user_id) on delete cascade
);

create index latex_overleaf_sync_links_resume_idx
  on private.latex_overleaf_sync_links (resume_id, origin_row_version)
  where resume_id is not null;
create index latex_overleaf_sync_links_version_idx
  on private.latex_overleaf_sync_links (resume_version_id, origin_row_version)
  where resume_version_id is not null;
create index latex_overleaf_sync_links_letter_idx
  on private.latex_overleaf_sync_links (cover_letter_id, origin_row_version)
  where cover_letter_id is not null;
create index latex_overleaf_sync_links_user_idx
  on private.latex_overleaf_sync_links (user_id);

alter table private.latex_overleaf_sync_links enable row level security;

create policy "Users can view own Overleaf sync links"
  on private.latex_overleaf_sync_links for select
  to authenticated
  using ((select auth.uid()) = user_id);

create policy "Users can create own Overleaf sync links"
  on private.latex_overleaf_sync_links for insert
  to authenticated
  with check ((select auth.uid()) = user_id);

revoke all on table private.latex_overleaf_sync_links from public, anon, authenticated;
grant usage on schema private to authenticated;
grant select, insert on table private.latex_overleaf_sync_links to authenticated;

create table private.latex_storage_cleanup_queue (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
  storage_path text not null,
  created_at timestamptz not null default now(),
  constraint latex_storage_cleanup_queue_path_check
    check (
      pg_catalog.length(storage_path) between 1 and 1024
      and storage_path !~ '(^|/)\.\.?(/|$)'
    ),
  constraint latex_storage_cleanup_queue_user_path_key
    unique (user_id, storage_path)
);

create index latex_storage_cleanup_queue_created_idx
  on private.latex_storage_cleanup_queue (user_id, created_at);

alter table private.latex_storage_cleanup_queue enable row level security;

create policy "Users can view own LaTeX cleanup queue"
  on private.latex_storage_cleanup_queue for select
  to authenticated
  using ((select auth.uid()) = user_id);

create policy "Users can create own LaTeX cleanup queue"
  on private.latex_storage_cleanup_queue for insert
  to authenticated
  with check ((select auth.uid()) = user_id);

create policy "Users can delete own LaTeX cleanup queue"
  on private.latex_storage_cleanup_queue for delete
  to authenticated
  using ((select auth.uid()) = user_id);

revoke all on table private.latex_storage_cleanup_queue from public, anon, authenticated;
grant select, insert, delete on table private.latex_storage_cleanup_queue to authenticated;

create table private.latex_overleaf_sync_cursors (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
  resume_id uuid,
  resume_version_id uuid,
  ...
  origin_row_version bigint not null,
  last_synced_row_version bigint not null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint latex_overleaf_sync_cursors_one_parent_check
    check (num_nonnulls(resume_id, resume_version_id, cover_letter_id) = 1),
  constraint latex_overleaf_sync_cursors_project_id_check
    check (overleaf_project_id ~ '^[A-Za-z0-9_-]{8,80}$'),
  constraint latex_overleaf_sync_cursors_manifest_version_check
    check (manifest_version = 1),
  constraint latex_overleaf_sync_cursors_versions_check
    check (
      origin_row_version >= 0
      and last_synced_row_version >= origin_row_version
    ),
  constraint latex_overleaf_sync_cursors_resume_user_fkey
    foreign key (resume_id, user_id)
    references public.resumes (id, user_id) on delete cascade,
  constraint latex_overleaf_sync_cursors_version_user_fkey
    foreign key (resume_version_id, user_id)
    references public.resume_versions (id, user_id) on delete cascade,
  constraint latex_overleaf_sync_cursors_cover_letter_user_fkey
    foreign key (cover_letter_id, user_id)
    references public.cover_letters (id, user_id) on delete cascade
);

create unique index latex_overleaf_sync_cursors_user_project_uidx
  on private.latex_overleaf_sync_cursors (user_id, overleaf_project_id);
create index latex_overleaf_sync_cursors_resume_idx
  on private.latex_overleaf_sync_cursors (resume_id)
  where resume_id is not null;
create index latex_overleaf_sync_cursors_version_idx
  on private.latex_overleaf_sync_cursors (resume_version_id)
  where resume_version_id is not null;
create index latex_overleaf_sync_cursors_letter_idx
  on private.latex_overleaf_sync_cursors (cover_letter_id)
  where cover_letter_id is not null;

alter table private.latex_overleaf_sync_cursors enable row level security;

create policy "Users can view own Overleaf sync cursors"
  on private.latex_overleaf_sync_cursors for select
  to authenticated
  using ((select auth.uid()) = user_id);

create policy "Users can create own Overleaf sync cursors"
  on private.latex_overleaf_sync_cursors for insert
  to authenticated
  with check ((select auth.uid()) = user_id);

create policy "Users can update own Overleaf sync cursors"
  on private.latex_overleaf_sync_cursors for update
  to authenticated
  using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);

revoke all on table private.latex_overleaf_sync_cursors from public, anon, authenticated;
grant usage on schema private to authenticated;
grant select, insert, update on table private.latex_overleaf_sync_cursors to authenticated;

create trigger latex_overleaf_sync_cursors_updated_at
  before update on private.latex_overleaf_sync_cursors
  for each row execute function public.set_updated_at();

create or replace function public.issue_latex_overleaf_sync_link(
  p_kind text,
  p_document_id uuid,
  p_origin_row_version bigint
)
returns uuid
language plpgsql
security invoker
...
begin
  ...
  if p_kind is null
    or p_kind not in ('master_resume', 'resume_version', 'cover_letter')
  then
    raise exception 'Invalid LaTeX document kind.' using errcode = '22023';
  end if;
  if p_origin_row_version is null or p_origin_row_version < 0 then
    raise exception 'Invalid LaTeX document version.' using errcode = '22023';
  end if;

  if p_kind = 'master_resume' then
    select d.row_version into v_row_version
  ...
  if v_row_version is null then
    raise exception 'The LaTeX document was not found.' using errcode = 'P0002';
  end if;
  if v_row_version <> p_origin_row_version then
    raise exception 'The LaTeX document changed while its project was exported.'
      using errcode = '40001';
  end if;
  ...
  where l.user_id = v_user_id
    and l.origin_row_version = v_row_version
    and (
      (p_kind = 'master_resume' and l.resume_id = p_document_id)
      or (p_kind = 'resume_version' and l.resume_version_id = p_document_id)
      or (p_kind = 'cover_letter' and l.cover_letter_id = p_document_id)
    )
  order by l.created_at desc
  limit 1;

  if v_link_id is null then
    insert into private.latex_overleaf_sync_links (
      user_id,
      resume_id,
      resume_version_id,
      cover_letter_id,
    ...
    values (
      v_user_id,
      case when p_kind = 'master_resume' then p_document_id end,
      case when p_kind = 'resume_version' then p_document_id end,
      case when p_kind = 'cover_letter' then p_document_id end,
      v_row_version
    )
...
revoke all on function public.issue_latex_overleaf_sync_link(
  text, uuid, bigint
) from public, anon;
grant execute on function public.issue_latex_overleaf_sync_link(
  text, uuid, bigint
) to authenticated;
...
begin
  ...
  if p_paths is null or pg_catalog.coalesce(pg_catalog.array_length(p_paths, 1), 0) > 100 then
  ...
  select pg_catalog.coalesce(pg_catalog.array_agg(q.id), array[]::uuid[])
    into v_ids
...
begin
  ...
  limit pg_catalog.least(
    pg_catalog.greatest(pg_catalog.coalesce(p_limit, 100), 1),
    100
  );
end;
$$;
...
begin
  ...
  if p_ids is null or pg_catalog.coalesce(pg_catalog.array_length(p_ids, 1), 0) > 100 then
...
begin
  ...
  select
    ...
    pg_catalog.coalesce(l.resume_id, l.resume_version_id, l.cover_letter_id),
    l.manifest_version,
  ...
  if v_kind = 'master_resume' then
    select d.row_version, pg_catalog.coalesce(d.content, ''), false
    ...
    select d.row_version, pg_catalog.coalesce(d.content, ''), d.submitted_at is not null
      into v_current_version, v_current_source, v_locked
    from public.resume_versions d
    where d.id = v_document_id
      and d.user_id = v_user_id
  ...
  v_expected_version := pg_catalog.coalesce(v_cursor_synced_version, v_origin_row_version);
  select
      ...
      left join public.latex_document_assets a
        ...
        and exists (
          ...
          where o.bucket_id = 'latex-workspaces'
            ...
            and pg_catalog.coalesce((o.metadata ->> 'size')::bigint, -1) = a.size_bytes
            and pg_catalog.coalesce(o.metadata ->> 'mimetype', '') = a.content_type
        )
      where a.id is null
    )
    into v_assets_match
...
begin
  ...
  if v_kind = 'master_resume' then
    select d.row_version, pg_catalog.coalesce(d.content, '')
  ...
  loop
    ...
    if not exists (
      ...
      where o.bucket_id = 'latex-workspaces'
        ...
        and o.owner_id = v_user_id::text
        and pg_catalog.coalesce((o.metadata ->> 'size')::bigint, -1) = v_size_bytes
        and pg_catalog.coalesce(o.metadata ->> 'mimetype', '') = v_content_type
    ) then
  ...
  if v_kind = 'master_resume' then
    ...
    select d.user_id, d.id, d.row_version, d.name, d.content_format, d.latex_engine,
      pg_catalog.coalesce(d.content, ''), 'overleaf_sync'
    from public.resumes d
    ...
    select d.user_id, d.id, d.row_version, pg_catalog.coalesce(d.title, 'Tailored resume'),
      d.content_format, d.latex_engine, pg_catalog.coalesce(d.content, ''), 'overleaf_sync'
    from public.resume_versions d
    where d.id = v_document_id and d.user_id = v_user_id;
  else
    insert into public.document_source_history (
