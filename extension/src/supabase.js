...
// TODO(phase-2): once the extension holds a real Supabase session (from the web
// auth-bridge or the popup login), send the logged-in user's access token as the
// Bearer instead of the anon key, so RLS ties rows to auth.uid():
