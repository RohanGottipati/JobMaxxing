...
// ── Auth gate ────────────────────────────────────────────────────────────────
// The dashboard is private: only load and show jobs when a session exists.
...
async function boot() {
  ...
  loginGate.style.display = loggedIn ? 'none' : 'flex';
  if (!loggedIn) return;

  // Load the dashboard once, the first time we become authenticated.
  if (!booted) {
    booted = true;
    loadPrefs();
    refresh();
...
// React to login/logout happening while the dashboard is open (e.g. the web
// login completes and the bridge pushes a session).
chrome.storage.onChanged.addListener((changes, area) => {
  if (area === 'local' && changes.jm_session) boot();
});
