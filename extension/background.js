...
import { getSession, setSession, clearSession } from './src/auth.js';
...
async function handleMessage(msg, sender) {
  switch (msg.type) {
    ...
    // Session handed in by the popup's own login (Phase 2) — stored as-is.
    case MSG.SET_SESSION: {
      await setSession(msg.session);
      return { ok: true };
    }

    // Session pushed from the JobMaxxing web app via the auth-bridge content
    // script after the user logs in there ("Log in with JobMaxxing" flow).
    case MSG.WEB_SESSION: {
      await setSession(msg.session);
      return { ok: true };
    }
